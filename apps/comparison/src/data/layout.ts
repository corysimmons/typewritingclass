import React from 'react'
import {
  cx, display, flex, flexCol, flexRow, inlineFlex, grid, gridCols, gap,
  relative, absolute, fixed, sticky, static_,
  visible, invisible, collapse_,
  z, overflow, overflowX, overflowY,
  aspectRatio, container, columns,
  float_, clear_, isolate, isolationAuto,
  objectFit, objectPosition,
  top, right, bottom, left, inset,
  boxSizing, p, bg, w, h, rounded, textColor, border, borderColor, css,
} from 'typewritingclass'
import type { ComparisonCategory } from './types.ts'

export const layoutData: ComparisonCategory = {
  title: 'Layout',
  description: 'Display, position, visibility, overflow, z-index, and more',
  sections: [
    {
      title: 'Display',
      examples: [
        {
          label: 'block',
          twcCode: 'cx(display("block"))',
          twcElement: React.createElement('div', { className: cx(display('block'), bg('#c7d2fe'), p(2)) }, 'block'),
          tailwindCode: 'block',
          tailwindElement: React.createElement('div', { className: 'tw-block tw-bg-indigo-200 tw-p-2' }, 'block'),
        },
        {
          label: 'inline-block',
          twcCode: 'cx(display("inline-block"))',
          twcElement: React.createElement('div', null,
            React.createElement('span', { className: cx(display('inline-block'), bg('#c7d2fe'), p(2)) }, 'inline-block'),
            React.createElement('span', { className: cx(display('inline-block'), bg('#a5b4fc'), p(2)) }, 'inline-block'),
          ),
          tailwindCode: 'inline-block',
          tailwindElement: React.createElement('div', null,
            React.createElement('span', { className: 'tw-inline-block tw-bg-indigo-200 tw-p-2' }, 'inline-block'),
            React.createElement('span', { className: 'tw-inline-block tw-bg-indigo-300 tw-p-2' }, 'inline-block'),
          ),
        },
        {
          label: 'inline',
          twcCode: 'cx(display("inline"))',
          twcElement: React.createElement('div', null,
            React.createElement('span', { className: cx(display('inline'), bg('#c7d2fe')) }, 'inline '),
            React.createElement('span', { className: cx(display('inline'), bg('#a5b4fc')) }, 'inline'),
          ),
          tailwindCode: 'inline',
          tailwindElement: React.createElement('div', null,
            React.createElement('span', { className: 'tw-inline tw-bg-indigo-200' }, 'inline '),
            React.createElement('span', { className: 'tw-inline tw-bg-indigo-300' }, 'inline'),
          ),
        },
        {
          label: 'flex',
          twcCode: 'cx(flex(), gap(2))',
          twcElement: React.createElement('div', { className: cx(flex(), gap(2)) },
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, 'A'),
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, 'B'),
          ),
          tailwindCode: 'flex gap-2',
          tailwindElement: React.createElement('div', { className: 'tw-flex tw-gap-2' },
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, 'A'),
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, 'B'),
          ),
        },
        {
          label: 'inline-flex',
          twcCode: 'cx(inlineFlex())',
          twcElement: React.createElement('div', null,
            React.createElement('span', { className: cx(inlineFlex(), gap(1), bg('#c7d2fe'), p(2)) },
              React.createElement('span', null, 'A'),
              React.createElement('span', null, 'B'),
            ),
            ' text after',
          ),
          tailwindCode: 'inline-flex',
          tailwindElement: React.createElement('div', null,
            React.createElement('span', { className: 'tw-inline-flex tw-gap-1 tw-bg-indigo-200 tw-p-2' },
              React.createElement('span', null, 'A'),
              React.createElement('span', null, 'B'),
            ),
            ' text after',
          ),
        },
        {
          label: 'grid',
          twcCode: 'cx(grid(), gridCols(2), gap(2))',
          twcElement: React.createElement('div', { className: cx(grid(), gridCols(2), gap(2)) },
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, '1'),
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, '2'),
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, '3'),
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, '4'),
          ),
          tailwindCode: 'grid grid-cols-2 gap-2',
          tailwindElement: React.createElement('div', { className: 'tw-grid tw-grid-cols-2 tw-gap-2' },
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, '1'),
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, '2'),
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, '3'),
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, '4'),
          ),
        },
        {
          label: 'hidden',
          twcCode: 'cx(display("none"))',
          twcElement: React.createElement('div', null,
            React.createElement('div', { className: cx(display('none')) }, 'Hidden'),
            React.createElement('div', { className: cx(bg('#c7d2fe'), p(2)) }, 'Visible (hidden element above)'),
          ),
          tailwindCode: 'hidden',
          tailwindElement: React.createElement('div', null,
            React.createElement('div', { className: 'tw-hidden' }, 'Hidden'),
            React.createElement('div', { className: 'tw-bg-indigo-200 tw-p-2' }, 'Visible (hidden element above)'),
          ),
        },
      ],
    },
    {
      title: 'Position',
      examples: [
        {
          label: 'relative',
          twcCode: 'cx(relative())',
          twcElement: React.createElement('div', { className: cx(relative(), bg('#c7d2fe'), p(4), h('4rem')) },
            React.createElement('div', { className: cx(absolute(), top(0), right(0), bg('#818cf8'), p(1)), style: { color: 'white', fontSize: '12px' } }, 'abs'),
            'relative parent',
          ),
          tailwindCode: 'relative',
          tailwindElement: React.createElement('div', { className: 'tw-relative tw-bg-indigo-200 tw-p-4 tw-h-16' },
            React.createElement('div', { className: 'tw-absolute tw-top-0 tw-right-0 tw-bg-indigo-400 tw-p-1 tw-text-white tw-text-xs' }, 'abs'),
            'relative parent',
          ),
        },
        {
          label: 'sticky (note: needs scroll context)',
          twcCode: 'cx(sticky(), top(0))',
          twcElement: React.createElement('div', { className: cx(sticky(), top(0), bg('#c7d2fe'), p(2)) }, 'sticky'),
          tailwindCode: 'sticky top-0',
          tailwindElement: React.createElement('div', { className: 'tw-sticky tw-top-0 tw-bg-indigo-200 tw-p-2' }, 'sticky'),
        },
      ],
    },
    {
      title: 'Visibility',
      examples: [
        {
          label: 'visible',
          twcCode: 'cx(visible())',
          twcElement: React.createElement('div', { className: cx(visible(), bg('#c7d2fe'), p(2)) }, 'visible'),
          tailwindCode: 'visible',
          tailwindElement: React.createElement('div', { className: 'tw-visible tw-bg-indigo-200 tw-p-2' }, 'visible'),
        },
        {
          label: 'invisible (takes space)',
          twcCode: 'cx(invisible())',
          twcElement: React.createElement('div', { className: cx(flex(), gap(2)) },
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, 'A'),
            React.createElement('div', { className: cx(invisible(), bg('#818cf8'), p(2)), style: { color: 'white' } }, 'B'),
            React.createElement('div', { className: cx(bg('#818cf8'), p(2)), style: { color: 'white' } }, 'C'),
          ),
          tailwindCode: 'invisible',
          tailwindElement: React.createElement('div', { className: 'tw-flex tw-gap-2' },
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, 'A'),
            React.createElement('div', { className: 'tw-invisible tw-bg-indigo-400 tw-p-2 tw-text-white' }, 'B'),
            React.createElement('div', { className: 'tw-bg-indigo-400 tw-p-2 tw-text-white' }, 'C'),
          ),
        },
      ],
    },
    {
      title: 'Overflow',
      examples: [
        {
          label: 'overflow-hidden',
          twcCode: 'cx(overflow("hidden"))',
          twcElement: React.createElement('div', { className: cx(overflow('hidden'), w('8rem'), h('3rem'), bg('#c7d2fe'), p(2)) }, 'This is a long text that will be clipped by overflow hidden'),
          tailwindCode: 'overflow-hidden',
          tailwindElement: React.createElement('div', { className: 'tw-overflow-hidden tw-w-32 tw-h-12 tw-bg-indigo-200 tw-p-2' }, 'This is a long text that will be clipped by overflow hidden'),
        },
        {
          label: 'overflow-auto',
          twcCode: 'cx(overflow("auto"))',
          twcElement: React.createElement('div', { className: cx(overflow('auto'), w('8rem'), h('3rem'), bg('#c7d2fe'), p(2)), style: { fontSize: '12px' } }, 'This is a long text that can be scrolled with overflow auto applied'),
          tailwindCode: 'overflow-auto',
          tailwindElement: React.createElement('div', { className: 'tw-overflow-auto tw-w-32 tw-h-12 tw-bg-indigo-200 tw-p-2 tw-text-xs' }, 'This is a long text that can be scrolled with overflow auto applied'),
        },
      ],
    },
    {
      title: 'Z-Index',
      examples: [
        {
          label: 'z-10 / z-20 stacking',
          twcCode: 'cx(z(10)), cx(z(20))',
          twcElement: React.createElement('div', { className: cx(relative(), h('4rem')) },
            React.createElement('div', { className: cx(absolute(), z(10), bg('#818cf8'), p(2), w('4rem'), h('3rem'), rounded('4px')), style: { color: 'white', fontSize: '12px', left: '0' } }, 'z-10'),
            React.createElement('div', { className: cx(absolute(), z(20), bg('#6366f1'), p(2), w('4rem'), h('3rem'), rounded('4px')), style: { color: 'white', fontSize: '12px', left: '2rem', top: '0.5rem' } }, 'z-20'),
          ),
          tailwindCode: 'z-10, z-20',
          tailwindElement: React.createElement('div', { className: 'tw-relative tw-h-16' },
            React.createElement('div', { className: 'tw-absolute tw-z-10 tw-bg-indigo-400 tw-p-2 tw-w-16 tw-h-12 tw-rounded tw-text-white tw-text-xs tw-left-0' }, 'z-10'),
            React.createElement('div', { className: 'tw-absolute tw-z-20 tw-bg-indigo-500 tw-p-2 tw-w-16 tw-h-12 tw-rounded tw-text-white tw-text-xs tw-left-8 tw-top-2' }, 'z-20'),
          ),
        },
      ],
    },
    {
      title: 'Aspect Ratio',
      examples: [
        {
          label: 'aspect-square',
          twcCode: 'cx(aspectRatio("1 / 1"), w("5rem"))',
          twcElement: React.createElement('div', { className: cx(aspectRatio('1 / 1'), w('5rem'), bg('#818cf8'), flex(), css({ 'align-items': 'center', 'justify-content': 'center' })), style: { color: 'white' } }, '1:1'),
          tailwindCode: 'aspect-square w-20',
          tailwindElement: React.createElement('div', { className: 'tw-aspect-square tw-w-20 tw-bg-indigo-400 tw-flex tw-items-center tw-justify-center tw-text-white' }, '1:1'),
        },
        {
          label: 'aspect-video',
          twcCode: 'cx(aspectRatio("16 / 9"), w("10rem"))',
          twcElement: React.createElement('div', { className: cx(aspectRatio('16 / 9'), w('10rem'), bg('#818cf8'), flex(), css({ 'align-items': 'center', 'justify-content': 'center' })), style: { color: 'white' } }, '16:9'),
          tailwindCode: 'aspect-video w-40',
          tailwindElement: React.createElement('div', { className: 'tw-aspect-video tw-w-40 tw-bg-indigo-400 tw-flex tw-items-center tw-justify-center tw-text-white' }, '16:9'),
        },
      ],
    },
    {
      title: 'Object Fit',
      examples: [
        {
          label: 'object-cover',
          twcCode: 'cx(objectFit("cover"))',
          twcElement: React.createElement('div', { className: cx(w('8rem'), h('4rem'), bg('#e0e7ff'), rounded('4px'), overflow('hidden')) },
            React.createElement('div', { className: cx(objectFit('cover'), w('100%'), h('100%'), bg('#818cf8')), style: { color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px' } }, 'cover'),
          ),
          tailwindCode: 'object-cover',
          tailwindElement: React.createElement('div', { className: 'tw-w-32 tw-h-16 tw-bg-indigo-100 tw-rounded tw-overflow-hidden' },
            React.createElement('div', { className: 'tw-object-cover tw-w-full tw-h-full tw-bg-indigo-400 tw-text-white tw-flex tw-items-center tw-justify-center tw-text-xs' }, 'cover'),
          ),
        },
        {
          label: 'object-contain',
          twcCode: 'cx(objectFit("contain"))',
          twcElement: React.createElement('div', { className: cx(w('8rem'), h('4rem'), bg('#e0e7ff'), rounded('4px'), overflow('hidden')) },
            React.createElement('div', { className: cx(objectFit('contain'), w('100%'), h('100%'), bg('#818cf8')), style: { color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px' } }, 'contain'),
          ),
          tailwindCode: 'object-contain',
          tailwindElement: React.createElement('div', { className: 'tw-w-32 tw-h-16 tw-bg-indigo-100 tw-rounded tw-overflow-hidden' },
            React.createElement('div', { className: 'tw-object-contain tw-w-full tw-h-full tw-bg-indigo-400 tw-text-white tw-flex tw-items-center tw-justify-center tw-text-xs' }, 'contain'),
          ),
        },
      ],
    },
    {
      title: 'Float & Clear',
      examples: [
        {
          label: 'float-right',
          twcCode: 'cx(float_("right"))',
          twcElement: React.createElement('div', { className: cx(bg('#e0e7ff'), p(2), overflow('hidden')) },
            React.createElement('div', { className: cx(float_('right'), bg('#818cf8'), p(2)), style: { color: 'white', fontSize: '12px' } }, 'float right'),
            React.createElement('span', { style: { fontSize: '12px' } }, 'Text wraps around the floated element on the left side.'),
          ),
          tailwindCode: 'float-right',
          tailwindElement: React.createElement('div', { className: 'tw-bg-indigo-100 tw-p-2 tw-overflow-hidden' },
            React.createElement('div', { className: 'tw-float-right tw-bg-indigo-400 tw-p-2 tw-text-white tw-text-xs' }, 'float right'),
            React.createElement('span', { className: 'tw-text-xs' }, 'Text wraps around the floated element on the left side.'),
          ),
        },
      ],
    },
    {
      title: 'Box Sizing',
      examples: [
        {
          label: 'box-border',
          twcCode: 'cx(boxSizing("border-box"))',
          twcElement: React.createElement('div', { className: cx(boxSizing('border-box'), w('8rem'), p(4), border('2px'), borderColor('#818cf8'), bg('#c7d2fe')), style: { fontSize: '12px' } }, 'border-box'),
          tailwindCode: 'box-border',
          tailwindElement: React.createElement('div', { className: 'tw-box-border tw-w-32 tw-p-4 tw-border-2 tw-border-indigo-400 tw-bg-indigo-200 tw-text-xs' }, 'border-box'),
        },
      ],
    },
  ],
}
